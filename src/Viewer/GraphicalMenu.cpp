...
void GraphicalMenu::create(std::vector<PanoInfo> panoList)
{
	...
	for (unsigned int i = 0; i < panoList.size(); ++i)
	{
		...
		thumbnails[i] = createTexture(13, width, height, GL_RGB, d);
...
void GraphicalMenu::display(glm::quat headsetRotation, glm::mat4x4 viewProjection, float radius, float panoSelection)
{
	...
	for (unsigned int i = minTile; i <= maxTile; ++i)
	{
		...
		shader.BindTexture("image", 13, thumbnails[i]); // The first 12 textures slots are used for panorama cube faces
		renderModel(quad);
	}
}